...
const
  ...
  // Values are grouped as count, totalCount, dataCount. Four rows per QR version: L, M, Q, H.
...
begin
  Result := Length(TEncoding.UTF8.GetBytes(S));
end;
...
begin
  ...
  if A.Num[0] = 0 then
    Exit(PolyMod(PolyCreateFromArray(A.Num, 0), B));
  if B.Num[0] = 0 then
    raise EInvalidOperation.Create('Invalid polynomial divisor');
...
begin
  ...
  Data := TEncoding.UTF8.GetBytes(FText);
...
begin
  ...
  ModuleSize := Min(ARect.Width, ARect.Height) div TotalModules;
  if ModuleSize < 1 then
    ModuleSize := 1;
  DrawSize := ModuleSize * TotalModules;
  OffsetX := ARect.Left + (ARect.Width - DrawSize) div 2;
  OffsetY := ARect.Top + (ARect.Height - DrawSize) div 2;
...
procedure TTMSQRCode.SaveToPNG(const AFileName: string);
var
  Bitmap: TBitmap;
  Png: TPngImage;
begin
