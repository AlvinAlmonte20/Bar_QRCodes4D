unit BarcodeTestMain;

interface

uses
  System.SysUtils, System.Classes, Vcl.Controls, Vcl.Forms, Vcl.StdCtrls,
  Vcl.ExtCtrls, Vcl.Dialogs, TMSBarcode;
...
procedure TMainForm.DisplayValueChanged(Sender: TObject);
begin
  UpdateBarcode;
end;
...
var
  AppFolder: string;
  PNGFileName, SVGFileName: string;
begin
  UpdateBarcode;

  AppFolder := IncludeTrailingPathDelimiter(ExtractFilePath(ParamStr(0)));
  PNGFileName := AppFolder + 'barcode.png';
  SVGFileName := AppFolder + 'barcode.svg';

  try
    FBarcode.SaveToPNG(PNGFileName);
    FBarcode.SaveToSVG(SVGFileName);
    ShowMessage(Format('Exported barcode to:%s%s%s%s',
      [sLineBreak, PNGFileName, sLineBreak, SVGFileName]));
  except
    on E: Exception do
...
function TMainForm.SampleValue(AType: TTMSBarcodeType): string;
begin
  case AType of
    tbcCODE39:
      Result := 'HELLO-39';
...
procedure TMainForm.UpdateBarcode;
begin
...
procedure TMainForm.ValueChanged(Sender: TObject);
begin
  UpdateBarcode;
end;
